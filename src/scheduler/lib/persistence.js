...
const fileValidator= Joi.array().items(
  Joi.object({
...
export async function loadSchedules() {
  ...
  try {
    ...
    if( e instanceof SyntaxError ) {
      throw new Error(`Could not load schedule from '${process.env.SCHEDULES_FILE}'. Invalid JSON file.`, {cause: e});
    }

    // The file does not exist yet -> This is fine
    if( e instanceof Error && e.code === 'ENOENT' ) {
      console.log(`No schedules found to load. File '${process.env.SCHEDULES_FILE}' does not exist`);
      return [];
    }
  ...
  if( error ) {
    throw new Error(`Could not load schedule from '${process.env.SCHEDULES_FILE}'. Invalid data: ${error.message}`, {cause: error})
  }
  ...
  return value.map( entry => new Schedule(
    entry.repoUuid,
    new Date( entry.nextRunDate ),
    entry.cadence
  ));
}
