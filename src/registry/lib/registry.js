...
class Service {
  ...
  addNotifySubscriber(serviceName, type, path) {
    ...
    if( alreadyExists ) {
      console.log(`Duplicate subscriber '${serviceName}' to service '${this.name}' (${type}/${path})`)
      return;
    }
  ...
  async broadcast( method, path, queryParams, body ) {
    ...
    for( const result of responseResults ) {
      if( result.status === 'rejected' ) {
        ...
        console.log(`Broadcast to '${result.value.url}' on service '${this.name}' returned with status ${result.value.status}`);
        success= false;
      }
    };
  ...
  async _notifySubscribers() {
    ...
    for(const subscriber of this.subscribers ) {
      ...
      if( !success ) {
        console.error(`Could not notify subscriber '${subscriber.serviceName}' of service '${this.name}' via ${subscriber.type}`);
      }
    }
  }
}
