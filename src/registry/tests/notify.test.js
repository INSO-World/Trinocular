...
describe('Notify Routes', () => {
  ...
  describe('postNotify', () => {
    it('should add a notify subscriber and send status 200', () => {
      ...
      expect(serviceStub.addNotifySubscriber.calledWith('subscriber1', 'type1', '/broadcastPath'))
        .to.be.true;
  ...
  describe('deleteNotify', () => {
    it('should remove a notify subscriber and send status 200', () => {
      ...
      expect(
        serviceStub.removeNotifySubscriber.calledWith('subscriber1', 'type1', '/broadcastPath')
      ).to.be.true;
      expect(res.sendStatus.calledWith(200)).to.be.true;
    });
    ...
    it('should return 404 if notifier is not found', () => {
      ...
      expect(
        res.end.calledWith(
          "Unknown notifier 'subscriber1/type1: /broadcastPath' on service 'service1'\n"
        )
      ).to.be.true;
    });
  });
