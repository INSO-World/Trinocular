...
describe('Routes', () => {
    let app, serverStub;
    const commonHeaders = {'Authorization': `bearer ${process.env.INTERNAL_API_SECRET}`}
    ...
    describe('DELETE /service/:name/notify/:subscriber/broadcast/*', () => {
        ...
        it('should return 404 if notifier is not found', async () => {
            ...
            await request(app)
                ...
                .expect(404)
                .expect(res => {
                    expect(res.text).to.equal("Unknown notifier 'subscriber1/broadcast: /somepath' on service 'service1'\n");
                });
        });
    });
