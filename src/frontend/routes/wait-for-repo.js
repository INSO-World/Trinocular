...
async function loadStatusInfo( repoUuid ) {
  ...
  switch( state ) {
    case 'pending': break;
    case 'error': apiBridgeStatus= repoStatus= visualizationStatus= 'error'; break;
    case 'done': apiBridgeStatus= repoStatus= visualizationStatus= 'done'; break;
    ...
    default:
      console.error(`Received unknown task state '${state}' from scheduler for task '${transactionId}'`);
      break;
  }
...
export async function getWaitPage(req, res) {
  ...
  // TODO: Load the repo name from the db
