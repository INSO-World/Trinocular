...
import {
  getDatasourceForRepoFromAPIService,
  getRepositoryFromRepoService
...
function prepareMemberGroups(gitRepoData, apiMembers, userUuid, repoUuid, mergingConfig) {
  ...
  const mergedMemberGroups = combineCurrentWithPreviousMemberGroups(
    mergingConfig || [],
    currentMemberGroups
  );
  ...
  mergedMemberGroups.forEach((contributors, memberName) =>
    memberGroupsArray.push({ contributors, memberName })
  );
...
export async function dashboard(req, res) {
  ...
  const [gitRepoData, apiMembers, milestones, repoDetails] = (dataSourceResponses =
    await Promise.all([
  ...
  if (dataSourceError) {
    logger.error(
      `Could not load common control data from one or more data sources: ${dataSourceError}`
    );
    return res.status(404).render('error', {
      user: req.user,
      isAuthenticated: req.isAuthenticated(),
  ...
  const memberGroups = prepareMemberGroups(
    gitRepoData,
    apiMembers,
    userUuid,
    repoUuid,
