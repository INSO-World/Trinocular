...
describe('GitView', () => {
  ...
  describe('authenticatedRemoteUrl()', () => {
    it('Should return authenticated remote URL', async () => {
      ...
      expect(url).to.equal(
        'https://dummy:glpat-sDqXSTfZsuXer5Bbzmaz@reset.inso.tuwien.ac.at/repo/2024ws-ase-pr-group/24ws-ase-pr-qse-07'
      );
    });
  });
  ...
  describe('getCommitInfoByHash()', () => {
    ...
    it('Should throw error about missing header information', async () => {
      ...
      await expect(gitView.getCommitInfoByHash(commitHash)).to.be.rejectedWith(
        Error,
        `Commit show of hash ${commitHash} is missing header info (author, date)`
      );
    });
    ...
    it('Should throw error about invalid format', async () => {
      ...
      await expect(gitView.getCommitInfoByHash(commitHash)).to.be.rejectedWith(
        Error,
        `Commit show of hash ${commitHash} has invalid diff format`
      );
    });
    ...
    it('Should throw error about date format', async () => {
      ...
      await expect(gitView.getCommitInfoByHash(commitHash)).to.be.rejectedWith(
        Error,
        `Commit show of hash ${commitHash} has invalid date format: '1734738517'`
      );
    });
    ...
    it('Should throw error about invalid diff format', async () => {
      ...
      await expect(gitView.getCommitInfoByHash(commitHash)).to.be.rejectedWith(
        Error,
        `Commit show of hash ${commitHash} has invalid diff format: 2\tsrc/frontend/public/style.css`
      );
    });
  ...
  describe('getAllContributors()', () => {
    it('Should return author emails as array', async () => {
      const authorString =
        '112  Christoph Neubauer <e12023172@student.tuwien.ac.at>\n' +
