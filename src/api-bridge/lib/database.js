...
export async function createTableIfNotExists( name, columns, options= {} ) {
  ...
  if( Array.isArray(options.compositePrimaryKey) ) {
    const keyColumnsString= options.compositePrimaryKey.map( key => pg.escapeIdentifier(key) ).join(',');
    constraintsString+= `, PRIMARY KEY (${ keyColumnsString })`;
  }

  await pool.query(`CREATE TABLE IF NOT EXISTS ${escapedName} (${columnsString} ${constraintsString})`);
}
...
export async function clearRepositoryRecordsFromTable( name, repoDbId ) {
  ...
  const result= await pool.query(`DELETE FROM ${escapedName} WHERE repository_id = $1`, [repoDbId]);
...
export async function loadAllRepositories() {
  ...
  const repositories= result.rows.map(
    ({name, uuid, id, type, auth_token, url}) => new Repository(name, uuid, id, type, auth_token, url)
  );
...
export async function getRecordsFromTable( name, repositoryDbId ) {
  ...
  const result= await pool.query(
    `SELECT * FROM ${escapedName} WHERE repository_id = $1`,
    [repositoryDbId]
  );
