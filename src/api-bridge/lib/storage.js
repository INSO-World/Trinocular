import { clearRepositoryRecordsFromTable, clearTable, createTableIfNotExists, getRecordFromTableById, getRecordsFromTable, insertRecordsIntoTable } from './database.js';
  ...
  async ensureTable( columns ) {
    ...
    for( const name in columns ) {
      if( !columns[name] || typeof columns[name] !== 'string' ) {
        throw Error(`Storage table declaration for endpoint '${this.endpoint}' has invalid column definition (${name} : ${columns[name]})`);
      }
    }
    
    // ID column must exist and be a primary key
    if( !columns.hasOwnProperty('id') ) {
      throw Error(`Storage table declaration for endpoint '${this.endpoint}' is missing an id column`);
    }

    if( columns.id.toLowerCase().includes('key') ) {
      throw Error(`Storage table declaration for endpoint '${this.endpoint}' has id column declared as key. (Key is generated automatically)`);
    }

    // The repository_id column name is reserved
    if( columns.hasOwnProperty('repository_id') ) {
      throw Error(`Storage table declaration for endpoint '${this.endpoint}' uses reserved column name 'repository_id'`);
    }
